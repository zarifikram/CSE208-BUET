...
public:
    ...
    bool MFEdmondsKarp(int nTeams, int source, int destination);
};
...
bool Graph::MFEdmondsKarp(int nTeams, int source, int destination){
...
// 2.1 Add bottleneck values to total value
    ...
    for(int i = 0; i < nTeams; i++) if(residualMatrix[i + 8][1]  != 0) return false;
...
int main()
{
    ...
    for(int tc = 0; tc < tt; tc++){
        ...
        for(int i = 0; i < m; i++){
            cin >> sz1 >> sz2;
            int num1 = szToNum(sz1);
            int num2 = szToNum(sz2);
            g.addEdge(num1 + 2, 2 + 6 + i, 1);
            g.addEdge(num2 + 2, 2 + 6 + i, 1);
        }

        for(int i = 0; i < m; i++) g.addEdge(6 + 2 + i, 1, 1);
        for(int i = 0; i < 6; i++) g.addEdge(0, 2 + i, n);

        
        cout << "Case " << tc + 1 << ": " << (g.MFEdmondsKarp(m, 0, 1) ? "YES" : "NO") << endl;
    }
}
